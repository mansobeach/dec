...
# 
# CVS: $Id: PriorityRulesSolver.rb,v 1.1 2009/02/25 07:21:10 decdev Exp $
#
# module ORC
#
#########################################################################
...
class PriorityRulesSolver
   ...
   def showQueue
      arrFiles = OrchestratorQueue::getQueuedFiles
      arrFiles.each{|aQueuedFile|
         puts aQueuedFile.filename
      }
   end
   #-------------------------------------------------------------
   ...
   def getSortedQueue2
      @sortedQueue = Array.new
      sortQueue
      @arrArrCandidates.each{|arrFiles|
         arrFiles.each{|aTrigger|
            @sortedQueue << aTrigger
         }
      }
      return @sortedQueue
   end

...
# 
# CVS: $Id: simHMI.rb,v 1.2 2009/03/17 15:40:48 decdev Exp $
#
#########################################################################
...
require 'cuc/ProcessHandler'

# Global variables
@@dateLastModification = "$Date: 2009/03/17 15:40:48 $"   # to keep control of the last modification
                                     # of this script
...
def main
   ...
   begin
      opts.each do |opt, arg|
         case opt      
            ...
            when "--version" then
               print("\nESA - Deimos-Space S.L.  DEC ", File.basename($0), " $Revision: 1.2 $  [", @@dateLastModification, "]\n\n\n")
               exit (0)
            when "--command" then
   ...
   trap("USR2")   {  
                     ...
                     if File.exist?("NRTP_STATUS_SUCCESS") == true then
                        puts "NRTP Job #{jobId} Execution was successfull ! :-)"
                        puts "======================================="
                     ...
                     if File.exist?("NRTP_STATUS_FAILED") == true then
                        puts "NRTP Job #{jobId} Execution FAILED ! :-("
                        puts "======================================="
                     ...
                     if File.exist?("NRTP_STATUS_ACK") == true then
                        puts "Message Acknowledge job #{jobId}, keep on waiting ! ;-)"
                        File.delete("NRTP_STATUS_ACK")
                        @sleepAgain = true
                        bProcessed  = true
                     end

                     if File.exist?("NRTP_STATUS_OFF") == true then
                        puts "NRTP is not running, lazy boy ! ;-p"
                        File.delete("NRTP_STATUS_OFF")
                        @sleepAgain = true
                        bProcessed  = true
                     end
                     ...
                     if bProcessed == false then
                        puts "No NRTP Status file found !! :-("
                     end

...
# 
# CVS: $Id: OrchestratorIngester.rb,v 1.4 2009/03/12 08:25:29 decdev Exp $
#
# module ORC
#
#########################################################################
   ...
   def ingest(arrPolledFiles)
      ...
      arrPolledFiles.each{|polledFile|
         ...
         if @ftReadConf.isValidFileType?(filetype) == true then
            ...
            bIngested = false
            @logger.debug("File-type #{filetype} not present in configuration !")
         end
         ...
         if bIngested == false then          
            ...
            if retVal == true then
               ...
               @logger.debug("Failed to move #{polledFile} to #{@orcTmpDir}/_ingestionError")

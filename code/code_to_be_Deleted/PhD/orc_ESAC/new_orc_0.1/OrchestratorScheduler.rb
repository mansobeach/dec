...
# 
# CVS: $Id: OrchestratorScheduler.rb,v 1.5 2009/03/18 11:48:15 decdev Exp $
#
# module ORC
#
#########################################################################
...
class OrchestratorScheduler
   ...
   def dispatch
      ...
      # Trigger the Job 
      @arrQueuedFiles.each{|queuedFile|                         
         cmd = "createJobOrderFile.rb -O -f #{queuedFile.filename} -i #{queuedFile.id} -L #{@procWorkingDir}"
         puts
         puts cmd
         @logger.debug("\n#{cmd}")
         begin

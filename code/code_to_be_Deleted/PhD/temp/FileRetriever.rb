...
# 
# CVS: $Id: FileRetriever.rb,v 1.18 2009/03/18 12:23:26 decdev Exp $
#
# module MINARC
#
#########################################################################
...
class FileRetriever
   ...
   def retrieve_by_type(destination, fileType, start = nil, stop = nil, bDelete = false, bIncStart = false, bIncStop = false, bHardlink = false)
      ...
      if arrFiles != nil and arrFiles.size > 0 then
         case @rule
            when "NEWEST" then arrFiles = keepOnlyNewest(arrFiles)
         ...
         arrFiles.each{|aFile|
            if @bListOnly then
               ...
               if @isDebugMode == true then
                  puts aFile.validity_start
                  puts aFile.validity_stop
                  puts
               end
               aRetVal = true
            else
               aRetVal = extractFromArchive(destination, aFile.filetype, aFile.filename, aFile.archive_date, bDelete, bHardlink)
   ...
   # It returns the list of files found with passed parameters
   def getFileList_by_type(fileType, start = nil, stop = nil, bIncStart = false, bIncStop = false)
   ...
   def getReferenceByName(filename)
      return ArchivedFile.find_by_filename(filename)
   end
   #-------------------------------------------------------------
   ...
   def retrieve_by_name(destination, filename, bDelete = false, bHardlink = false)
      ...
      # aFile = ArchivedFile.fixDate(aFile)
      ...
      if @bListOnly then
         ...
         retVal = extractFromArchive(destination, aFile.filetype, aFile.filename, aFile.archive_date, bDelete, bHardlink)
...
private
   ...
   def extractFromArchive(destination, fileType, fileName, archDate, bDelete, bHardlink)

      arrEntries = Array.new

      srcDir = "#{@archiveRoot}/#{fileType}/" << archDate.strftime("%Y%m")

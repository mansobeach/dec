...
def main
   ...
   cmd = "verifyMeteoExcel.rb -s #{meteoFilename} -t _Verified_#{meteoFilename}"
   ...
   if File.exist?("_Verified_#{meteoFilename}") == true then
      cmd = "excel2Chart.rb -e _Verified_#{meteoFilename} -p #{prefixImage}"
   else
   ...
   if File.exist?("_Verified_#{meteoFilename}") == true then
      ...
      cmd = "\\mv -f _Verified_#{meteoFilename} ../excel"
...
def removePrevDayFiles
   ...
   time           = Time.now
   time           = time - 86400
   yesterdayFile  = time.strftime("%Y%m%dT")

   ...
   begin 
      opts.each do |opt, arg|
         case opt      
            ...
            when "--version" then
               print("\nESA - Deimos-Space S.L.  DEC ", File.basename($0), " $Revision: 1.15 $  [", @@dateLastModification, "]\n\n\n")
               exit (0)
...
#    end 
   ...
   if aROP.transferability.to_i == @@NOT_TRANSFERABLE then
   ...
   arrInv = InventoryROPFileView.find_by_sql(["select FILENAME, FILE_ID, TYPE_ID from ROP_FILE_VW
                                                 where CURRENT_ROP=? and TYPE_ID<? and status=?",@nROP, 1000, @@STATUS_VALIDATED ])
...
def getNamesFromIDs(arrIDs)   
   ...
   arrIDs.each{|x|
      aFilename = InventoryFile.find_by_file_id(x)
      if aFilename != nil then 
         arrFilename << aFilename.filename
      else
         puts
...
def getFileFromRPFArchive2MainOutbox(file)
   ...
   if nameReader.fileContent.upcase == ".EEF" or
      ...
      command = %Q{removeSchema.bin -f #{targetFile}}
      ret = system(command)
      if ret == false then
         puts

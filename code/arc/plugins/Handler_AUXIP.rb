...
# This class allows minarc to handle Copernicus S1/S2 files:
#
# S1A_TEST_MPL_ORBSCT_20140403T224609_99999999T999999_0006.EOF
#
...
class Handler_AUXIP
   ...
   def initialize (name, destination = nil, args = {})
      ...
      if @filename.slice(0,2) == "S1" then
         @source_prefix = "S1"
      end

      if @filename.slice(0,2) == "S2" then
         @source_prefix = "S2"
      end

      if @filename.slice(0,3) == "EDR" then
         @source_prefix = "S2"
      end

      if @filename.slice(0,3) == "DEC" &&
         @filename.include?("S1") == true &&
         @filename.include?("AUIP") == true then
         @source_prefix = "S1"
      end

      if @filename.slice(0,3) == "DEC" &&
         @filename.include?("S2") == true &&
         @filename.include?("AUIP") == true then
         @source_prefix = "S2"
      end
      ...
      @archive_path  = "#{archRoot}/#{@source_prefix}/#{@type}/#{Date.today.strftime("%Y")}/#{Date.today.strftime("%m")}/#{Date.today.strftime("%d")}"

...
#
# == Usage
# listenerFT.rb  --all [-R]| --mnemonic <MNEMONIC> --interval <seconds>
...
#
# CVS: $Id: listenerFT.rb,v 1.8 2008/07/03 11:38:07 decdev Exp $
#
#########################################################################
...
def main
   ...
   # initialize logger
   loggerFactory = CUC::Log4rLoggerFactory.new("listenerFT", "#{ENV['DCC_CONFIG']}/dec_log_config.xml")
   if @isDebugMode then
   ...
   if @logger == nil then
      puts
		puts "Error in listenerFT.rb::main"
...
#   retVal = execute(command, "listenerFT")
...
def launchListener(entity)
   ...
   if @isDebugMode == true then
      command  = %Q{listenerFT.rb --mnemonic #{entity} --interval #{pI} -D}
   else
      command  = %Q{listenerFT.rb --mnemonic #{entity} --interval #{pI}}
   end
   puts command
...
def stopGetFromEntity(mnemonic)
   checker = CUC::CheckerProcessUniqueness.new("getFromInterface.rb", mnemonic, true)
   pid     = checker.getRunningPID
   if pid == false then
      puts "There was not a getFromInterface for #{mnemonic} I/F running !"
   else
      puts "Sending signal SIGKILL to Process #{pid} for killing getFromInterface.rb -m #{mnemonic}"
      @logger.info("Abort current polling Operations ...")
      Process.kill(9, pid.to_i)
	   checker.release
...
def checkModuleIntegrity
   ...
   if bDefined == false then
      puts "\nError in listenerFT::checkModuleIntegrity :-(\n\n"
      exit(99)
   end                             
end
#-------------------------------------------------------------

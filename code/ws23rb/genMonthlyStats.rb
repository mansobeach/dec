...
def main
   ...
   excelFilename  = %Q{METEO_MONTHLY_STATS_temperature_outdoor_#{currentDay.slice(0,6)}.xls}
   chartFilename  = %Q{METEO_STATS_#{currentDay.slice(0,6)}_temperature_outdoor.png}

   prevDir     = Dir.pwd
   confDir     = ENV['METEO_CONFIG']
   ...
   # initialize logger
   loggerFactory = CUC::Log4rLoggerFactory.new("genMonthlyStats", "#{confDir}/meteo_log_config.xml")
   if @isDebugMode then
   ...
   if @logger == nil then
      puts
	   puts "Error in genMonthlyStats::main"
		puts "Could not set up logging system !  :-("
      puts "Check Meteo logs configuration under \"#{confDir}/dec_log_config.xml\"" 
		puts
		puts
		exit(99)
   end

   @logger.info("Generating Monthly Charts ...")
   ...
   # archive monthly file

   currDir = Dir.pwd

   cmd = "minArcStore.rb -f #{currDir}/#{excelFilename} -d -u -t METEO_MONTHLY_XLS"

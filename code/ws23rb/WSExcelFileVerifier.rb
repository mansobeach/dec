   ...
   def verify_temperature
      @hiColVbles.each{|field,column|
        ...
        @sheet.each{|row|
           ...
           # First Row is the header with no data
           if bFirst == true then
              bFirst = false
              next
           end
           ...
           # Second Row is used only to initialise the time segment & first value
           ...
           if currTime.to_i > prevTime.to_i + 300 then
              ...
              lastTrendValue = oldValue
              lastTrendTime  = prevTime
              idxGap    = row.idx
              bGapped   = true
              next
           end
           ...
           if (currValue - prevValue).abs > thresholdTemp then
              if bGapped == true then
                 bGapped = false
                 if (row.idx-1) == idxGap then
                    if (prevValue - oldValue).abs > maxThreshold then
                       @hErrors[row.idx-1] = column
                       puts "#{field} [#{oldValue} , #{prevValue}] [#{lastTrendTime.strftime("%H:%M:%S")} , #{prevTime.strftime("%H:%M:%S")}] is out of trend #{maxThreshold}"
                    end
                 end  
              end
              puts "#{field} [#{prevValue} , #{currValue}] [#{prevTime.strftime("%H:%M:%S")} , #{currTime.strftime("%H:%M:%S")}] is out of trend #{thresholdTemp}"
              bOutTrend = true
              if @hErrors[row.idx] == nil then
                 @hErrors[row.idx] = Array.new
              end
              @hErrors[row.idx]  << column
              currValue          = prevValue
           else
              bOutTrend = false
              # lastTrendValue is not used

...
def main
   ...
   @logger.debug("Generating Daily Charts ...")
...
#   outboxDir   = ENV['METEO_OUTBOX']
   ...
   tempDir     = %Q{#{ENV['METEO_TEMP']}/.#{str}_#{stationName}_DAILY_CHARTS}  
...
#   Dir.chdir(time.strftime("%d"))
   ...
   if File.exist?(meteoVFilename) == true then
      ...
      @logger.fatal("Could not find #{meteoVFilename}")
      exit(99)
   ...
   arrFiles.each{|aFile|
      ...
      todayFile   = %Q{DAILY_#{vble}_#{stationName}_TODAY.png}
      ...
      # Duplicate files with name DAILY_TEMPERATURE-OUTDOOR_CASALE_TODAY.png

      cmd = "\\cp -f #{newFile}  #{todayFile}"
   ...
   # Send chart files (png)

   cmd = "send2Interface.rb -m METEO -D --nodb"
   system(cmd)
...
def init  
   ...
   # initialize logger
   loggerFactory = CUC::Log4rLoggerFactory.new("genDailyMeteoFile", "#{ENV['METEO_CONFIG']}/meteo_log_config.xml")
   if @isDebugMode then
   ...
   if @logger == nil then
      puts
	   puts "Error in genDailyMeteoFile::main"

...
# 
# CVS: $Id: Orchestrator.rb,v 1.2 2009/01/09 11:20: decdev Exp $
#
# module ORC
#
#########################################################################
...
#    end
   ...
   @ingestorChecker  = CUC::CheckerProcessUniqueness.new("ingesterComponent.rb", "", true)
...
def start 
   ...
   if @ingestorChecker.isRunning == false then
      ...
      pid = @ingestorChecker.getRunningPID
...
def processStatus
   ...
   if @ingestorChecker.isRunning == false then 
      ...
      puts "Ingester component is running with pid #{@ingestorChecker.getRunningPID}"
      # @logger.debug("There is a daemon running for the ingestor component with pid #{@ingestorChecker.getRunningPID}")
   end
...
def stop
   ...
   if @ingestorChecker.isRunning == true then
      pid = @ingestorChecker.getRunningPID

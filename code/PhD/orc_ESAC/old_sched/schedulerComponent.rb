...
#
# CVS: $Id: schedulerComponent.rb,v 1.8 2008/12/19 11:38:07 decdev Exp $
#
#########################################################################
...
# Global variables
@@dateLastModification = "$Date: 2008/12/19 11:38:07 $"   
...
def main
   ...
   # initialize logger
   loggerFactory = CUC::Log4rLoggerFactory.new("schedulerComponent", "#{@orcConfigDir}/orchestrator_log_config.xml")
...
def start
   ...
   @logger.debug("STARTING orchestrator scheduler daemon")
   ...
   listener = CUC::Listener.new(File.basename($0), "", 0, @OrcSch.method("start").to_proc)       
   
   trap("SIGUSR1") {                          
                     @logger.debug("Calling scheduler from ingestor")
                     @OrcSch.signalHandler("usr1")               
                   }     

   trap("SIGUSR2") {  @OrcSch.signalHandler("usr2")  }                                         

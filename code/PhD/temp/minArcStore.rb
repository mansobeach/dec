...
#
# CVS: $Id: minArcStore.rb,v 1.14 2009/03/26 09:21:32 decdev Exp $
#
#########################################################################
...
# Global variables
@@dateLastModification = "$Date: 2009/03/26 09:21:32 $"   # to keep control of the last modification
                                       # of this script
...
def main
   ...
   archiver  = MINARC::FileArchiver.new(@bMove, @bHardLink)
   ...
   if @full_report_name != "" then
      if @full_report_name.index('/') == nil or @full_report_name.index('/') != 0 then
         ...
         archiver.enableReporting(@full_report_name)
      end
   end

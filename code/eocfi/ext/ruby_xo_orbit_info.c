                           ...
                           ) 
{
   ...
   if (iDebug == 1)
   {
      printf("DEBUG: ENTRY method_xo_orbit_info\n") ;
   }
   ...
   if (TYPE(abs_orbit_) == T_FIXNUM)
      ...
      rb_fatal("method_xo_orbit_info abs_orbit is not a long type compatible (T_FIXNUM) but %i", TYPE(abs_orbit_)) ;
   ...
   if (iDebug == 1)
   {
      printf("DEBUG: method_xo_orbit_info xo_orbit_info status: %ld ierr: %li\n", status, *ierr) ;
   }
   ...
   if (iDebug == 1)
   {
      printf("DEBUG: EXIT method_xo_orbit_info\n") ;  
   }

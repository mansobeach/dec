...
cmd="sudo docker image build -t app_minarc_s2 . -f install/docker/Dockerfile.decservice.minarc.s2.cloudferro"
echo $cmd
eval $cmd
...
cmd="sudo docker image build -t app_dec_s2 . -f install/docker/Dockerfile.s2decservice.dec"
echo $cmd
eval $cmd
